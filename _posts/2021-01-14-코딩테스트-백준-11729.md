...
<br/>    
    <pre>2.<strong>재귀 코드 내에서 더해가는 방법</strong></pre><br/>
    <pre>* 출력 순서가 횟수, 이동순서 순으로 출력이 되야 하기 때문에, 재귀함수 내에서 바로 print문으로 출력할 수가 없습니다.</pre><br/>
    <pre>* 다음과 같은 프로세스로 재귀함수를 바꿔줍니다.</pre>
    <pre>1. <strong>base condition</strong></pre><br/>
    <pre>n = 1일때, 1과 [start_point, end_point]를 return 해줌.</pre><br/>
    <pre>2. <strong>재귀함수 내부 프로세스</strong><br/>
        <pre><pre>1. n-1까지의 원판을 2 즉 (6-a-b)로 옮기고, 옮긴 횟수와 옮기는 과정을 받아옴. <i>편의를 위해 a = start_point, b = end_point로 대체</i></pre><br/>
        <pre><pre>2. n번째 원판을 b로 옮김 (횟수 + 1, 과정 추가)</pre></pre><br/>
        <pre><pre>3. n-1까지의 원판을 (6-a-b)에서 3으로 옮기고 횟수와 과정을 받아옴</pre></pre><br/>
        <pre><pre>4. 횟수와 과정을 모두 더해서 return</pre></pre><br/>
<br/>
